...
export class FeedbackComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFeedback): number => this.feedbackService.getFeedbackIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IFeedback[] | null): IFeedback[] {
    ...
    if (this.links().prev) {
      ...
      if (data) {
        for (const d of data) {
          if (feedbacksNew.map(op => op.id).indexOf(d.id) === -1) {
            feedbacksNew.push(d);
          }
        }
      }
